...
bool ciWMFVideoPlayer::loadMovie( const std::wstring& filePath, const wstring& audioDevice )
{
	if( !mPlayer ) {
	...
	HRESULT hr = S_OK;
	const std::wstring& w = filePath;

	hr = mPlayer->OpenURL( w.c_str(), audioDevice.c_str() );
